...
import { loadEdgerBilling, type EdgerBillingSlice, type PlanId } from "../lib/edgerBillingLocal";
import { splitFullNameForClerk } from "./auth/fullNameDerived";
...
export default function Profile() {
  ...
  useEffect(() => {
    const sync = () => setBilling(loadEdgerBilling());
    sync();
    const onVis = () => {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-[220px_1fr] gap-10 md:gap-14">
            ...
            <article className="min-w-0">
              {activeTab === "account" && (
                <AccountSection displayName={displayName} email={email} onNameSave={handleNameSave} />
              )}
...
function BillingSection({ billing }: { billing: EdgerBillingSlice }) {
      ...
      <div className="gradient-frame-mint frame-grain rounded-3xl p-1.5 relative overflow-hidden border border-white/40 shadow-[0_30px_60px_-30px_rgba(0,0,0,0.18)]">
        <div className="bg-white rounded-[20px] p-6 md:p-7">
          ...
          <div className="mb-5">
            ...
            <p className="text-zinc-600 text-[14px] leading-relaxed">{PLAN_DESCRIPTION[billing.plan]}</p>
          </div>
          ...
          {isPayg ? (
            <div className="bg-zinc-50 rounded-xl border border-zinc-100 px-4 py-3.5 mb-5 flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="font-mono text-base text-zinc-700 tabular-nums">${(billing.credits * 0.2).toFixed(2)}</div>
              </div>
            </div>
          ) : billing.plan === "free" ? (
            <div className="bg-zinc-50 rounded-xl border border-zinc-100 px-4 py-3.5 mb-5 flex items-center justify-between">
              <div>
                ...
                <div className="font-mono text-2xl font-medium tabular-nums tracking-[-0.02em] text-zinc-950">
                  {billing.credits} <span className="text-zinc-400 text-base">/ 5</span>
                </div>
              </div>
              <div className="text-right">
                <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-500 mb-1">
            ...
            <div className="bg-zinc-50 rounded-xl border border-zinc-100 px-4 py-3.5 mb-5">
              <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-500 mb-1">AI analyses</div>
              <div className="font-mono text-2xl font-medium tabular-nums tracking-[-0.02em] text-zinc-950">
...
function UsageSection({ billing }: { billing: EdgerBillingSlice }) {
  const isPayg = billing.plan === "payg";
    ...
    <div className="space-y-5">
        ...
        {isFree && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mt-4 flex items-start gap-3">
            ...
            <p className="text-[13px] text-amber-900 leading-relaxed">
              ...
              <Link to="/pricing" className="underline underline-offset-2 hover:text-amber-700">
                Compare plans
              </Link>
              .
            </p>
      ...
      <SectionCard kicker="History" title="Recent calculations" sub="The last lot-size calculations you ran.">
        <p className="font-mono text-[12px] text-zinc-500 italic">
          ...
          <Link to="/app" className="underline underline-offset-2 hover:text-zinc-900">
            Analyzer
          </Link>{" "}
...
function SectionCard({
  ...
  return (
    ...
    >
      <div>
        <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-500 font-medium">{kicker}</span>
...
function UsageStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-zinc-50 rounded-xl border border-zinc-100 px-4 py-3.5">
      <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-500 mb-1">{label}</div>
      <div className="font-mono text-xl font-medium tabular-nums tracking-[-0.02em] text-zinc-950">{value}</div>
    </div>
  );
}
